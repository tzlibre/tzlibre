...
module Script = struct
  ...
  let exn_shell msg res =
    Tezos_client_001_Havana.Proto_alpha.Alpha_environment.wrap_error res
    |> exn_tezos msg

  let parse exprs =
    Tezos_client_001_Havana.Michelson_v1_parser.(
    ...
    | Ok json ->
        let repr =
          Tezos_data_encoding.Data_encoding.Json.destruct
            Tezos_client_001_Havana.Proto_alpha.Script_repr.encoding json
        in
        let ( (expr_code :
                Tezos_client_001_Havana.Proto_alpha.Michelson_v1_primitives.prim
                Tezos_micheline.Micheline.canonical)
            , _ ) =
          Tezos_client_001_Havana.Proto_alpha.Script_repr.(force_decode repr.code)
          |> exn_shell "decoding script-repr"
        in
        let module Alph = Tezos_client_001_Havana.Proto_alpha in
        let strings_node =
  ...
  let json_script_repr code storage =
    match
      Tezos_data_encoding.Data_encoding.Json.construct
        Tezos_client_001_Havana.Proto_alpha.Script_repr.encoding
        Tezos_client_001_Havana.Proto_alpha.Script_repr.
...
let default () =
  ...
  ; expected_pow= 1
  ; name= "alpha"
  ; hash= "PsFGmCdDaB6KffJm333qXXC89MbzE5t4orDYWNHtNkZNAwYMYje"
