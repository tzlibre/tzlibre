...
activate_alpha() {
    ...
    ${client} \
        -block genesis \
        activate protocol PsFGmCdDaB6KffJm333qXXC89MbzE5t4orDYWNHtNkZNAwYMYje \
        with fitness 1 \
        and key activator \
...
alias tezos-activate-alpha="$client  -block genesis activate protocol PsFGmCdDaB6KffJm333qXXC89MbzE5t4orDYWNHtNkZNAwYMYje with fitness 1 and key activator and parameters $parameters_file --timestamp $(TZ='AAA+1' date +%FT%TZ)" ;
...
command, is "Ps8LKGP9YR66KH9iyvhhycv91VcEiUowpbj1TfLB6iYGmXUtA69", you
