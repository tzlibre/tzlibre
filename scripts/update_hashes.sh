...
current_hash_genesis=`jq '.hash' < src/proto_000_Ps8LKGP9/lib_protocol/src/TEZOS_PROTOCOL | tr -d '"'`
echo "Genesis's current hash: $current_hash_genesis"
genesis_tmpdir=`mktemp -d`
mkdir $genesis_tmpdir/src
cp src/proto_000_Ps8LKGP9/lib_protocol/src/*.ml src/proto_000_Ps8LKGP9/lib_protocol/src/*.mli $genesis_tmpdir/src/
grep -v '"hash"' < src/proto_000_Ps8LKGP9/lib_protocol/src/TEZOS_PROTOCOL > $genesis_tmpdir/src/TEZOS_PROTOCOL
